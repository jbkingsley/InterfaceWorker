...
abstract class Mapper
{
     ...
     * @return array 数据结果
     */
    abstract protected function doFind($id, Service $service = null, $collection = null);
     ...
     * @param string $key
     * @return mixed
     * @throws ServerInternalException 指定的配置不存在
     */
    public function getOption($key)
    {
        if (!isset($this->options[$key])) {
            throw new ServerInternalException('Mapper: undefined option "'.$key.'"');
        }
    ...
    public function getService()
    {
        ...
        if(empty($service)==true){
            throw new ServerInternalException('not set service option');
        }
        if(Functions::DI()->$service==null){
            throw new ServerInternalException('DI() dbServer not register');
        }
        return Functions::DI()->$service;
    }
    ...
    {
        if ($this->isReadonly()) {
            throw new ServerInternalException($this->class.' is readonly');
        }
        ...
        if (!$result) {
            throw new ServerInternalException($this->class.' save failed');
        }
    ...
    public function destroy(Data $data)
    {
        ...
        if (!$this->doDelete($data)) {
            throw new ServerInternalException($this->class.' destroy failed');
        }
