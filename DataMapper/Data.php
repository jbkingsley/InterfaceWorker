...
use InterfaceWorker;
use InterfaceWorker\DataMapper;
...
abstract class Data implements JsonSerializable
{
     ...
     * @var string
     */
    protected static $mapper = '\InterfaceWorker\DataMapper\Mapper';
    ...
    public function __call($method, array $args)
    {
        ...
        if ($prefix != 'set' && $prefix != 'get') {
            throw new ServerInternalException(sprintf('Call undefined method %s:%s()', get_class($this), $method));
        }
        ...
        if (!$this->has($key)) {
            ...
            if (!$found) {
                throw new ServerInternalException(get_class($this).": Undefined property {$key}");
            }
        }
    ...
    public function setIn($key, $path, $value, $push = false)
    {
        ...
        if (!is_array($target)) {
            throw new ServerInternalException(get_class($this).": Property {$key} is not complex type");
        }

        \InterfaceWorker\array_set_in($target, $path, $value, $push);
    ...
    public function unsetIn($key, $path)
    {
        ...
        \InterfaceWorker\array_unset_in($target, $path);
        $this->change($key, $target);
    ...
    public function getIn($key, $path)
    {
        ...
        return \InterfaceWorker\array_get_in($target, $path);
    }
        ...
        foreach ($keys as $key) {
            ...
            if ($type->isNull($value)) {
                if (!$attribute['allow_null']) {
                    throw new ServerInternalException(sprintf('%s: Property "%s", not allow null', get_class($this), $key));
                }
            } else {
                if ($attribute['regexp'] && !preg_match($attribute['regexp'], $value)) {
                    throw new ServerInternalException(sprintf('%s: Property "%s", mismatching pattern %s', get_class($this), $key, $attribute['regexp']));
                }

                if (!$attribute['allow_tags'] && \InterfaceWorker\str_has_tags($value)) {
                    throw new ServerInternalException(sprintf('%s: Property "%s", cannot contain tags', get_class($this), $key));
                }
                ...
                try {
                    ...
                    throw new ServerInternalException($message, 0, $ex);
                }
            }
        }
    ...
    protected function prepareSet($key, $force = false)
    {
        ...
        if ($attribute['deprecated']) {
            throw new ServerInternalException(get_class($this).": Property {$key} is deprecated");
        } elseif (!$force && $attribute['refuse_update'] && !$this->isFresh()) {
            throw new ServerInternalException(get_class($this).": Property {$key} refuse update");
        }
    ...
    public static function findOrFail($id)
    {
        ...
        throw new ServerInternalException('no data');
    }
