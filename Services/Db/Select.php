...
class Select
{
    ...
    /**
     * @param \InterfaceWorker\Services\DB\Adapter $adapter
     ...
     * 获取数据库连接.
     *
     * @return \InterfaceWorker\Services\DB\Adapter
     */
     ...
     * 执行查询，返回查询结果句柄对象
     *
     * @return \InterfaceWorker\Services\Db\Statement
     */
    ...
    public function setProcessor($processor)
    {
        if ($processor && !is_callable($processor)) {
            throw new ServerInternalException('Select processor is not callable');
        }
    ...
    public function delete()
    {
        ...
        // 不允许没有任何条件的delete
        if (!$where) {
            throw new ServerInternalException('MUST specify WHERE condition before delete');
        }

        // 见方法注释
        if ($this->limit or $this->offset or $this->group_by) {
            throw new ServerInternalException('CAN NOT DELETE while specify LIMIT or OFFSET or GROUP BY');
        }
    ...
    public function update(array $row)
    {
        ...
        // 不允许没有任何条件的update
        if (!$where) {
            throw new ServerInternalException('MUST specify WHERE condition before update');
        }

        // 见delete()方法注释
        if ($this->limit or $this->offset or $this->group_by) {
            throw new ServerInternalException('CAN NOT UPDATE while specify LIMIT or OFFSET or GROUP BY');
        }
