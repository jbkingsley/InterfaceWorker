...
abstract class Adapter extends Service
{
    ...
    {
        if (!isset($config['dsn'])) {
            throw new ServerInternalException ('Invalid database config, require "dsn" key.');
        }
    ...
    public function connect()
    {
        ...
        $options[PDO::ATTR_STATEMENT_CLASS] = ['\InterfaceWorker\Services\DB\Statement'];
        ...
        try {
            ...
            throw new ServerInternalException('Database connect failed!', 0, $exception);
        }
    ...
    public function begin()
    {
        if ($this->in_transaction) {
            if (!$this->support_savepoint) {
                throw new ServerInternalException(get_class($this).' unsupport savepoint');
            }
