...
namespace InterfaceWorker\Services;

use InterfaceWorker\Exceptions\ServerInternalException;
use InterfaceWorker\Service;
use InterfaceWorker\Validator;

use Redis;
if (!extension_loaded('redis')) {
    throw new \Exception('Require redis extension!');
}

class RedisAdapter extends Service
{
    ...
    public function connect()
    {
        ...
        if (!$conn) {
            throw new ServerInternalException('Cannot connect redis');
        }

        if ($config['password'] && !$handler->auth($config['password'])) {
            throw new ServerInternalException('Invalid redis password');
        }

        if ($config['database'] && !$handler->select($config['database'])) {
            throw new ServerInternalException('Select redis database['.$config['database'].'] failed');
        }
