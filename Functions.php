...
class Functions{
     ...
     * 获取DI
     * 相当于Injection::one()
     * @return \InterfaceWorker\Injection
     */
    public static function DI() {
        return Injection::one();
    }
     ...
     * @return bool
     */
    public static function str_has_tags($string)
    {
    ...
    public static function array_set_in(array &$target, array $path, $value, $push = false)
    {
        ...
        foreach ($path as $key) {
            ...
            if (!is_array($target)) {
                throw new ServerInternalException('Cannot use a scalar value as an array');
            }
        }
     ...
     *
     */
    public static function array_push_in(array &$target, array $path, $value)
    {
        return array_set_in($target, $path, $value, true);
    }

    public static function array_get_in(array $target, array $path)
    {
        foreach ($path as $key) {
            if (!isset($target[$key])) {
                return false;
    ...
    public static function array_unset_in(array &$target, array $path)
    {
        $last_key = array_pop($path);
     ...
     * @return array;
     */
    public static function array_trim($target)
    {
    ...
    public static function safe_json_encode($value, $options = 0, $depth = 512)
    {
        ...
        if ($value === false && json_last_error() !== JSON_ERROR_NONE) {
            throw new ServerInternalException(json_last_error_msg(), json_last_error());
        }

        return $value;
    }

    public static function safe_json_decode($json, $assoc = false, $depth = 512, $options = 0)
    {
...
class Validator
{
    ...
    protected function checkScalar($key, $value, array $rule)
    {
        ...
        } elseif (!$rule['allow_tags'] && Functions::str_has_tags($value)) {
    ...
    {
        try {
            $value = Functions::safe_json_decode($value, true);
    ...
    private function exception($key, $message)
    {
        ...
        $exception            = new ServerInternalException($message);
