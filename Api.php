...
class Api {
     ...
     * 获取规则解析后的接口参数
     * @param string $name 接口参数名字
     * @throws ServerInternalException 获取未设置的接口参数时，返回500
     * @return mixed
     */
    public function __get($name) {
        if(!isset($this->$name) || empty($name)) {
            throw new ServerInternalException(
                "InterfaceWorker\\Api::${$name} undefined"
            );
        }
        return $this->$name;
    }
     ...
     * - 3、[可选]用户身份验证
     *
     * @uses core_cloud_Api::createMemberValue()
     * @uses core_cloud_Api::filterCheck()
     * @uses core_cloud_Api::userCheck()
     * @return null
     */
     ...
     *
     * @param $inPath array
     * @uses core_cloud_Api::getApiRules()
     */
    protected function createMemberValue() {
        foreach ($this->getApiRules() as $key => $rule) {
            $this->$key = Functions::DI()->request->getByRule($rule);
        }
    }
     ...
     * - 1、实现api_filter::check()接口
     * - 2、注册的过滤器到DI()->filter
     *
     * <br>以下是一个简单的示例：<br>
    ```
     * 	class My_Filter implements Filter {
     *
    ...
    protected function filterCheck() {
        $filter = Functions::DI()->get('filter', 'InterfaceWorker\Filter\None');

        if (isset($filter)) {
            if (!($filter instanceof Filter)) {
                throw new BadRequestException(
                    "DI()->filter should be instanceof Filter");
            }
            $filter->check();
        }
    }
